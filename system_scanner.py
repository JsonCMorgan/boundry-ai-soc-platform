...
# Max concurrent host probes during network discovery (prevents thread exhaustion).
MAX_NETWORK_SCAN_THREADS = 50


# ─── Security: finding_id allow-list ───────────────────────────────────────
#
# Every finding_id generated by this module follows the pattern
# [A-Za-z0-9_]{1,64} — see how each `_check_*` builder constructs them
# (e.g. "firewall_off_domain", "open_port_445", "pwd_never_expires_jason",
# "smb_share_data", "net_192_168_1_1_3389").
#
# Because finding_ids are later interpolated into PowerShell command
# strings inside get_remediation_plan(), we MUST reject anything outside
# this character set before it reaches subprocess. Without this gate, a
# tampered DB row or future API caller could plant metacharacters such as
# ';', '&', '|', '`', '$', quotes, or newlines and achieve command
# injection — even though _run_ps uses the argv-list form (no shell=True),
# PowerShell still parses the whole `-Command` string and would execute
# the injected payload.
#
# The regex is intentionally narrow: lowercase + uppercase ASCII letters,
# digits, and underscores, 1-64 chars. No dots, dashes, or whitespace.
_FINDING_ID_RE = re.compile(r"^[A-Za-z0-9_]{1,64}$")


def _validate_finding_id(finding_id: str) -> None:
    """
    Raise ValueError if finding_id contains anything outside the strict
    allow-list. Defence-in-depth against PowerShell command injection.
    """
...
def get_remediation_plan(finding_id):
    ...
    Security: finding_id is validated against a strict allow-list before
    any string component is interpolated into a PowerShell command, so
    metacharacters like ';', '&', '|', '`', '$', quotes, or newlines can
    never reach subprocess. See _validate_finding_id().
...
def verify_finding_fixed(finding_id):
    ...
    Security: finding_id is validated by get_remediation_plan() via
    _validate_finding_id() before any PowerShell command is built.
    """
    _validate_finding_id(finding_id)
...
def run_network_scan():
    ...
    alive_hosts = []
    lock        = threading.Lock()
    with ThreadPoolExecutor(max_workers=MAX_NETWORK_SCAN_THREADS) as pool:
        pool.map(lambda ip: _ping_host(ip, alive_hosts, lock), all_hosts[:254])
